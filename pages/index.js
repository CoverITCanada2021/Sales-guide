...
export default function SalesPricingApp() {
  ...
  const crewSize = 3;
  const travelCost = distanceKm > 250 ? (crewSize * 250 * weeksNeeded) : 0;
  ...
  const blockTravelCost = distanceKm > 250 ? crewSize * 250 * blockInstallDays : 0;
  ...
  return (
    <div className="p-6 font-sans max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label>Distance from Home Base (km): </label>
          <input type="number" value={distanceKm} onChange={(e) => setDistanceKm(+e.target.value)} className="border px-2 py-1 w-full" />
        </div>
        <div>
      ...
      <div className="mt-8 bg-gray-100 p-6 rounded-lg shadow">
        ...
        {distanceKm > 250 && <p>Travel/Accommodation Cost (Dome): ${travelCost.toFixed(2)}</p>}
        ...
        {distanceKm > 250 && <p>Travel/Accommodation Cost (Block): ${blockTravelCost.toFixed(2)}</p>}
